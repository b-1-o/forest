...
function Reveal({ children, className = '', delay = 0 }: { children: ReactNode; className?: string; delay?: number }) {
  return <motion.div className={className} initial={{ opacity: 0, y: 50, filter: 'blur(12px)' }} whileInView={{ opacity: 1, y: 0, filter: 'blur(0px)' }} viewport={{ once: true, margin: '-12% 0px' }} transition={{ duration: 0.9, delay, ease: [0.16, 1, 0.3, 1] }}>{children}</motion.div>
}
...
function App() {
  ...
  useEffect(() => {
    ...
    const update = () => { const y = window.scrollY; const vh = window.innerHeight || 1; target?.style.setProperty('--scroll', `${y}px`); target?.style.setProperty('--scroll-progress', `${Math.min(y / vh, 12)}`); frame = 0 }
    const onScroll = () => { if (!frame) frame = requestAnimationFrame(update) }
    const onMouse = (event: MouseEvent) => { setMouse({ x: event.clientX, y: event.clientY }); target?.style.setProperty('--mouse-x', `${event.clientX}px`); target?.style.setProperty('--mouse-y', `${event.clientY}px`) }
    update(); window.addEventListener('scroll', onScroll, { passive: true }); window.addEventListener('mousemove', onMouse, { passive: true })
    return () => { window.removeEventListener('scroll', onScroll); window.removeEventListener('mousemove', onMouse); if (frame) cancelAnimationFrame(frame) }
  }, [])
  useEffect(() => {
    const elements = ['home', ...navItems].map((id) => document.getElementById(id)).filter(Boolean) as HTMLElement[]
    const observer = new IntersectionObserver((entries) => { const visible = entries.filter((entry) => entry.isIntersecting).sort((a, b) => b.intersectionRatio - a.intersectionRatio)[0]; if (visible) setActive(visible.target.id) }, { rootMargin: '-35% 0px -55% 0px', threshold: [0, 0.15, 0.4] })
    elements.forEach((element) => observer.observe(element)); return () => observer.disconnect()
  }, [])
  const goTo = (id: string) => { setActive(id); document.getElementById(id)?.scrollIntoView({ behavior: 'smooth', block: 'start' }) }

  return <div className="site" style={{ '--mouse-x': `${mouse.x}px`, '--mouse-y': `${mouse.y}px` } as CSSProperties}>
    <a className="skip" href="#about">Skip to content</a>
    <AnimatePresence>{intro && <motion.div className="intro" exit={{ opacity: 0, scale: 1.025, filter: 'blur(12px)' }} transition={{ duration: 0.9, ease: [0.16, 1, 0.3, 1] }}><div className="intro-line" /><span>DIGITAL / WEB / UI</span><h1>Into the quiet.</h1><div className="intro-loader"><motion.i initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 2.2, ease: [0.16, 1, 0.3, 1] }} /></div><small>THE WORLD MOVES SLOWLY</small></motion.div>}</AnimatePresence>
    <div className="world" ref={worldRef} aria-hidden="true"><div className="world-layer world-back"><img src={forestImage} alt="" /></div><div className="world-layer world-mid"><img src={forestImage} alt="" /></div><div className="world-layer world-near"><img src={forestImage} alt="" /></div><div className="world-light" /><div className="world-fog world-fog-one" /><div className="world-fog world-fog-two" /><div className="world-vignette" /><div className="world-cursor" /></div>
    <header className="nav"><button className="brand" onClick={() => goTo('home')} aria-label="Back to top"><span className="brand-mark">+</span><small>WEB / UI</small></button><nav aria-label="Primary navigation">{navItems.map((item) => <button key={item} className={active === item ? 'active' : ''} onClick={() => goTo(item)}>{item}</button>)}</nav><a className="nav-cta" href="https://www.fiverr.com/s/Q27bB5p" target="_blank" rel="noreferrer">START <ArrowUpRight size={13} /></a></header>
    <main>
      <section id="home" className="hero"><div className="hero-copy"><motion.p className="eyebrow" initial={{ opacity: 0, y: 20 }} animate={{ opacity: intro ? 0 : 1, y: intro ? 20 : 0 }} transition={{ duration: 0.8 }}>WEB DEVELOPER / UI / DIGITAL</motion.p><motion.h2 initial={{ opacity: 0, y: 60, filter: 'blur(18px)' }} animate={{ opacity: intro ? 0 : 1, y: intro ? 60 : 0, filter: intro ? 'blur(18px)' : 'blur(0px)' }} transition={{ delay: 0.15, duration: 1.15, ease: [0.16, 1, 0.3, 1] }}>I make small<br /><em>businesses</em> look<br />like they mean it.</motion.h2><motion.p className="hero-description" initial={{ opacity: 0 }} animate={{ opacity: intro ? 0 : 1 }} transition={{ delay: 0.55, duration: 0.8 }}>Clean interfaces. Responsive websites. Thoughtful motion.<br />Built to be useful, fast and unmistakably yours.</motion.p><motion.button className="enter-button" initial={{ opacity: 0, y: 25 }} animate={{ opacity: intro ? 0 : 1, y: intro ? 25 : 0 }} transition={{ delay: 0.8, duration: 0.8 }} onClick={() => goTo('about')}><span>ENTER THE WORK</span><ArrowDown size={15} /></motion.button></div><div className="hero-bottom"><span>34.1866° N / 118.4390° W</span><span><i /> SCROLL TO EXPLORE</span></div></section>
